(async ()=>{
  ...
  try {
    ...
    const ms = new Date(now.getTime() - 5*60000).toISOString();
    const me = new Date(now.getTime() + 30*60000).toISOString();
    const as = new Date(now.getTime() + 60*60000).toISOString();
    const ae = new Date(now.getTime() + 120*60000).toISOString();

    const body = {
      title: 'session-now-node',
      description: 'immediate session for testing',
      startDate: start,
      endDate: end,
      morningStart: ms,
      morningEnd: me,
